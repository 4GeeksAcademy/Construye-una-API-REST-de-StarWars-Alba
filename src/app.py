...
def get_all_users():
    stmt = select(Users)
    users= db.session.execute(stmt).scalars().all() #como es todos devuelve una colección de los registros
    return jsonify([user.serialize()for user in users]), 200  #se pone el serialize xq es una lista?? es lista xq son todos los usuarios?
...
@app.route("/people", methods=["POST"]) #da error
...
@app.route("/people/<int:id>", methods=["DELETE"]) #da error
